...
async function getYouTubeAuthCookies() {
    return new Promise(resolve => {
        chrome.cookies.getAll({ domain: "youtube.com" }, yt => {
            chrome.cookies.getAll({ domain: "google.com" }, gl => {
                resolve([...(yt || []), ...(gl || [])]);
            });
        });
    });
    ...
    if (message.type === "PREPARE_DISPATCH_DOWNLOAD") {
        ...
        if (pageUrl.includes("youtube.com") || pageUrl.includes("youtu.be")) {
            ...
            if (pageUrl.startsWith("http")) {
                // Use the new base domain fetcher instead of exact URL fetcher
...
async function triggerDownload(url, filename, referer, cookies, youtubeQuality, forceHLS) {
    // If no cookies are provided (e.g. from context menu or standard intercept), grab the base domain cookies automatically
